...
export class RoleService {
  ...
  public async RoleAuth(role: string, module_name: string, feature_name: string) {
    const filter = {
      $and: [{ name: role }, { module: module_name }, { features: feature_name }],
    };
  ...
  async update(params: UpdateRoleDto) {
    return await RoleModel.findOneAndUpdate({ name: params.role, module: params.module }, params, { new: true });
  }
}
